...
test("release gateは未追跡payloadを拒否し、ignore済み成果物だけを除外する", async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), "aishell-release-gate-"));
  ...
  git(work, "config", "user.name", "AIShell release gate test");
  git(work, "config", "user.email", "aishell-release-gate@example.invalid");
  ...
  await rm(path.join(work, "untracked.txt"));
