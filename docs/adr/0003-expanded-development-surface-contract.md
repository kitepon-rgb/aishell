...
実測traceはpositiveなら期待toolちょうど1回、no-callなら0回だけを成功とする。schema上必要なIDやprofileを得るため、
期待toolより前の補助callは、probeの禁止候補でない場合だけ許す。期待toolより後のcall、期待toolの重複、禁止候補は
余計なcallまたは誤routingとして失敗させる。discovery modelへはproductionのtool名・順・日英description・instructionsを渡し、
schema互換はwire/schema testが別に担うため、discovery harnessでは引数を空objectへ正規化して実処理を起動しない。
native shellは9本同士の選択を汚染するためこのgateだけ無効にし、shellとの競合はPhase 7のtool adoptionで実測する。
