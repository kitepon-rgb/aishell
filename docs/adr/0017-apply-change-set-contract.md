...
- `client_id`: owner登録後にclient stateへdurable保存するstable canonical lowercase UUID v4文字列。
  `^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$`だけを受理し、nil UUID、uppercase、別表記を拒否する。
- `client_epoch`: `1...9_007_199_254_740_991`のJSON integer。owner承認付きrotation時だけ1増やす。
- `request_sequence`: `1...9_007_199_254_740_991`のJSON integer。同じclient内で新規requestごとに1ずつ増やす。
- `cursor`: ADR 0004の`ws2` cursor。対象root、exclusion規則、generation、呼出側が確認したjournal位置を束縛する。
...
`apply_change_set`はclient slotを暗黙作成しない。固定slotに存在しない`client_id`は`CHANGE_SET_CLIENT_NOT_REGISTERED`とする。
...
client slotのallocation、epoch rotation、retireは公開MCP toolではなく、macOS管理appのlocal owner control planeだけが行うH操作である。
...
- `transaction_id`、`client_id`、`client_epoch`、`request_sequence`、`status: committed`、`visibility`、`root`、`from_cursor`、更新後`cursor`。
