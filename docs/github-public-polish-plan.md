...
- [ ] 5秒で用途とDirect OSの差分が伝わるREADME冒頭、30秒で試せる導線、比較表を用意する。
- [ ] 硬派なmacOS開発toolに合うhero / Social previewを生成し、過剰な装飾や風化しやすいUI表現を避ける。
- [ ] description、topics、homepage、Discussions、Community Profile、security設定を意図どおり揃える。
- [ ] npm package、default branch、tag、GitHub Release、release notesの対応を検証可能にする。
- [ ] ローカル変更、GitHub設定変更、手動作業、未解決事項を分けて報告する。
...
- [ ] 承認済みのtag / Release操作だけを実行し、release notesと実commitを再確認する。
- [x] Social preview用JPEGをSettings UIへuploadし、custom OG URLをAPIで確認する。

## Phase 5: 最終受入

- [ ] GitHub公開ページ、README、Release、Community Profileを再監査する。
- [ ] 変更ファイル、検証結果、スキップ理由、手動残作業を報告する。
- [ ] 完了後、このplanを`docs/archive/`へ移す。
...
- Swift実装を変更しないため、`swift test`は実行しない。
