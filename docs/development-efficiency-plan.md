...
updated: 2026-07-19
status: initial-surface-implemented-formal-benchmark-pending
owner: AIShell
...
2026-07-19時点で初期高密度5 tool、default development profile、full互換profile、隔離benchmark runnerまで実装済み。途中の候補runはbinary・prompt条件を混成していたため正式比較から棄却した。最終candidateの同一binary・同一manifestによる3 sentinel × 3反復を取り直している。30 task代表suite、semantic context、product targetは未着手であり、本roadmap全体の完了扱いにはしない。
...
- [ ] development profileの高密度toolが公開されている。
...
- [ ] native Codex baselineに対して成功率を維持する。
- [ ] 代表suiteの`tokens per solved task`でproduct targetを達成する。
- [ ] wall time gateを満たす。
- [ ] 既存20 primitiveの内部化または互換方針が完了している。
- [ ] README、設計文書、RAG、release notesが実装と一致している。
