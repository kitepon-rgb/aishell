...
## Phase 7 pre-measurement binding amendment（2026-07-23）

ACE-070の測定attemptを1件も開始する前に、実行時まで存在しないmanaged run IDとarmごとに異なるretained
artifact IDを、全arm同一の固定値として要求していた矛盾を検出した。task、fixture、mutation、oracle、model-visible
goal、反復数、arm順、集計式は変更せず、次のbindingだけを修正する。

- `run_observe.run_id`はsetup値でなく、同一attemptの`run_check`成功結果を指す`$run_check.run_id` data-dependency tokenとする。
- artifact比較promptは同一templateを維持し、harnessが各armの検証済みopaque run IDだけを実行直前に埋める。
- adapter traceはtemplate SHA、実run ID、raw request/result bytes、射影bytesを一対一に束縛する。
- 旧materializer SHA `0d9ebb7d1a5c565ec6476ef723dc5a0e96589a35291227d24cbd3d521747b4a3`を再利用しない。
- 現materializerは5293 bytes、SHA-256 `189da2b4473cce55123382edd4f67d0a9113f62c89167516a9c789a992910860`。

Phase 7 production runnerは32 task × 3 arm × 3反復＝288 attemptをexactに要求し、prefix checkpointからだけ
再開する。invalid attemptは置換せず、provider SSE usage、実model snapshot、candidate adapter trace、非timeout・exit 0を
全attemptで検証する。
