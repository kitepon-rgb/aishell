...
let result = await importWorkerReport({ cwd, control_id, actor_id, expected_revision: 118, worker_run_id, report });
const verification = "親再検証: 前run親検証済み2ファイルとbyte一致、retry ChangeImpactServiceTests 8/8、git diff --check成功";
const evidence = {
  type: "command",
  ref: "parent-focused-verification:ace034-impact-continuation-retry",
  digest: createHash("sha256").update(verification).digest("hex"),
  observed_at: "2026-07-22T06:14:20.000Z",
};
result = await accept({
