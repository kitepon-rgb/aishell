...
import { accept, importWorkerReport, taskFinalizeRecord } from "/Users/kite/Developer/dotagents/lib/orchestrate/control-record.mjs";
...
const worker_run_id = "ace034-impact-continuation-writer-run";
const report = JSON.parse(fs.readFileSync(`${cwd}/.lattice/import/delegation/${worker_run_id}/worker-report.json`, "utf8"));
let result = await importWorkerReport({ cwd, control_id, actor_id, expected_revision: 114, worker_run_id, report });
const verification = "親再検証: ChangeImpactServiceTests 8/8、OpaqueContinuation 4/4、git diff --check成功";
const evidence = {
  type: "command",
  ref: "parent-focused-verification:ace034-impact-continuation-seam",
  digest: createHash("sha256").update(verification).digest("hex"),
  observed_at: "2026-07-22T06:08:31.000Z",
};
result = await accept({
  cwd, control_id, actor_id, expected_revision: result.revision, worker_run_id,
  result_digest: report.result_digest, verification_evidence: [evidence],
