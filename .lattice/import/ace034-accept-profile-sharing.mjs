...
const worker_run_id = "ace034-profile-sharing-writer-run";
const report = JSON.parse(fs.readFileSync(
  `${cwd}/.lattice/import/delegation/${worker_run_id}/worker-report.json`, "utf8"
));
let result = await importWorkerReport({
  cwd, control_id, actor_id, expected_revision: 111, worker_run_id, report,
});
const verification = "親再検証: ContextCompilerServiceTests 11/11、git diff --check成功";
const evidence = {
  type: "command",
  ref: "parent-focused-verification:ace034-profile-service-sharing",
  digest: createHash("sha256").update(verification).digest("hex"),
  observed_at: "2026-07-22T06:07:38.000Z",
};
result = await accept({
