...
最初に「許可rootを追加」でAIに操作させる範囲を選ぶ。絶対パスはいずれかのrootへ自動的に対応し、相対パスは一覧の先頭rootを基準にする。停止中は通常のファイル・アプリ・process操作を拒否するが、`runtime_status` と `runtime_open_manager` は利用できる。
...
```text
AIShellを使いこなして。最初にruntime_statusを確認し、停止中または許可root不足ならruntime_open_managerで管理画面を開いて、私に画面上の変更を案内して。絶対パスはいずれかの許可root、相対パスはprimaryAllowedRootPathを基準に扱って。
```

`runtime_status` は複数root、相対パスの基準、停止状態、次の操作を返す。停止中でも `runtime_open_manager` で管理アプリを前面化できるため、Computer Useや旧shellで入口を作る必要はない。許可root変更と再開は管理画面で人が行い、停止中の通常操作は引き続き拒否される。
